...
disaster_recovery_folder="./production"
...
if [ ! -d "$production_folder" ] || [ ! -d "$disaster_recovery_folder" ]; then
    ...
    [ ! -d "$production_folder" ] && missing_folders+=("Production")
    [ ! -d "$disaster_recovery_folder" ] && missing_folders+=("Disaster Recovery")

    if [ ${#missing_folders[@]} -eq 1 ]; then
        echo "Error: ${missing_folders[0]} folder does not exist."
    elif [ ${#missing_folders[@]} -eq 2 ]; then
        echo "Error: Production and Disaster Recovery folders do not exist."
    else
        echo "Error: Some required folders do not exist."
    fi
    ...
    if [ -d "$subdirectory" ]; then
        ...
        if [ -d "$disaster_recovery_subdirectory" ]; then
            ...
            for production_yaml_file in "$subdirectory"/*.yaml; do
                # Extract the corresponding file name without -teller and -crm
                base_name=$(basename "$production_yaml_file" | sed -E 's/(-teller|-crm)//g')

                # Check if the disaster recovery file exists
                disaster_recovery_yaml_file="$disaster_recovery_subdirectory/$base_name"
